...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Archiving {len(all_files)} files with status: {status}")
        ...
        logger.exception("Error archiving files")
        raise
...
def archive_successful_files(bucket, file_list, batch_id):
    ...
    for file_key in file_list:
        try:
            ...
            logger.info(f"Archived successful file: {file_key} → {archive_key}")

        except Exception as e:
            logger.error(f"Error archiving successful file {file_key}: {str(e)}")
...
def archive_failed_files(bucket, file_list, batch_id, error_info):
    ...
    for file_key in file_list:
        try:
            ...
            logger.info(f"Archived failed file: {file_key} → {archive_key}")

        except Exception as e:
            logger.error(f"Error archiving failed file {file_key}: {str(e)}")
    ...
    try:
        ...
        logger.info(f"Stored error details at: {error_log_key}")
    except Exception as e:
        logger.error(f"Error writing error log for batch {batch_id}: {str(e)}")
...
def update_processed_files_status(file_list, batch_id, status):
    ...
    try:
        ...
        logger.info(f"Updated {len(file_list)} files with status: {status}")

    except Exception as e:
        logger.error(f"Error updating processed files table: {str(e)}")
